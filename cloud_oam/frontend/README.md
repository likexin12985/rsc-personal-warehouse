...
客户端只开放已经验收的身份、会话、权限上下文、省负责人配置和正式库存状态只读；v0.9 历史页面不能替代
审批、分配、占用、出库、发货、物流签收、OAM 收货、个人仓入库、通知送达或对账同步。

正式首页在 `/auth/me` 与 `/access/context` 的人员、授权版本、账号/在职状态、运营角色及
`inventory/read` 权限完全一致后，才请求 `/v1/inventory/summary`。响应必须符合 V1 契约；
后端 `0010` 已建立期初证据表，并有一个不挂路由的内部启动服务用于固定 scope、控制快照、
零正式账本快照、冻结和首轮任务；`0011`–`0015` 及内部首轮实盘服务已能封存完整范围实物、显式空范围、
逐件 SN、待核实观察和整轮提交，并以标识类型、显式封轮 completion、不可变审计和完整证据链防止
别名重复与重放篡改，但没有挂载正式 API，待核实处理、复盘、双复核和终结仍未完成，
不能产生可信成立事实。因此客户端仍只展示账本游标和“期初未建立”，不读取 v0.9 余额、不把
启动任务、首轮已提交、表存在或单笔 `opening` 当作已开账、不显示假零值，也不跨 SKU/计量
单位汇总。所有 v0.9 业务 GET
与写路径以及 fragment、编码分隔符、dot-segment、绝对 URL 等非规范 API 路径均在发起
网络请求前失败关闭。
