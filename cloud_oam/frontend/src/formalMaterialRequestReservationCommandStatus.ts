...
function exact(value: unknown, keys: readonly string[], name = "预约命令状态"): Record<string, unknown> {
...
function normalizeCommand(value: unknown): MaterialRequestReservationCommand {
  const command = exact(value, [
    ...
    "stock_account_id", "reserve_transaction_id", "reserve_transaction_no", "reserved_qty",
  ...
  if (command.reservation_status !== "reserved" || command.idempotency_replayed !== true) {
    return fail("预约命令状态轴无效");
  }
  if (states.request_status !== command.request_status) return fail("预约命令申请状态与状态轴不一致");
  if (states.reservation_status !== "pending" && states.reservation_status !== "reserved") {
    return fail("预约命令占用状态轴无效");
  }
  if (typeof command.reserved_qty !== "string" || !QUANTITY.test(command.reserved_qty)) {
    return fail("预约数量无效");
  }
  return {
...
export function validateMaterialRequestReservationCommandStatus(
  ...
  if (row.schema_version !== "1.0") return fail("预约命令状态版本无效");
  if (row.lookup_status === "not_observed") {
    if (row.command !== null) return fail("未观察到预约命令时不能携带事实");
    return { schema_version: "1.0", lookup_status: "not_observed", command: null };
  }
  if (row.lookup_status !== "confirmed" || row.command === null) return fail("预约命令状态无效");
...
export function validateMaterialRequestReservationMutationResult(
  ...
  if (typeof raw.idempotency_replayed !== "boolean") return fail("预约写响应幂等标记无效");
