...
export function createFormalMaterialRequestAdapter(
  ...
  return Object.freeze({
    ...
    receiptCommandStatusNoReplay(requestId: string, key: string) {
      if (!SAFE_COORDINATE.test(key) || key.length < 16 || key.length > 128) throw new ApiError(400, "收货核验坐标无效");
      return requireNoReplayRequester()<unknown>(`/v1/material-requests/${requiredUuid(requestId, "request_id")}/receipt-command-status`, {
        cache: "no-store", headers: { "Idempotency-Key": key, "Cache-Control": "no-store", Pragma: "no-cache" },
      }).then(validateReceiptCommandStatus);
    },
    ...
    createInboundOrder(requestId: string, input: InboundOrderInput, headers: Readonly<{ "X-Request-ID": string }>) {
      const body = validateInboundOrderInput(input); return requireNoReplayRequester()<unknown>(`/v1/material-requests/${requiredUuid(requestId, "request_id")}/inbound-orders`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, ...jsonBody(body) }).then(validateInboundOrderResult);
    },
