...
function MaterialOperationsTab({ workOrderId }: { workOrderId: string }) {
  ...
  return <div className="work-order-tab-panel"><div className="form-actions"><Button tone="secondary" icon={<RefreshCw size={16} />} disabled={loading} onClick={load}>刷新履约记录</Button></div><div className="table-wrap"><table><thead><tr><th>操作号</th><th>操作类型</th><th>过账事务</th><th>状态</th></tr></thead><tbody>{history.items.map((item) => <tr key={item.operation_id}><td className="mono">{item.operation_no}</td><td>{({ consume: "消耗", release: "释放", occupy: "占用", recover: "收回", reverse: "冲销" } as Record<string, string>)[item.operation_type] || item.operation_type}</td><td className="mono">{item.posting_transaction_id}</td><td>{item.status === "posted" ? "已过账" : item.status}</td></tr>)}</tbody></table></div></div>;
}
