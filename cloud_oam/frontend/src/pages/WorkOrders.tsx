...
function MaterialOperationsTab({ workOrderId }: { workOrderId: string }) {
  ...
  return <div className="work-order-tab-panel"><div className="form-actions"><Button tone="secondary" icon={<RefreshCw size={16} />} disabled={loading} onClick={load}>刷新履约记录</Button></div><div className="table-wrap"><table><thead><tr><th>操作号</th><th>操作类型</th><th>物料明细</th><th>过账事务</th><th>状态</th></tr></thead><tbody>{history.items.map((item) => <tr key={item.operation_id}><td className="mono">{item.operation_no}</td><td>{materialOperationLabel(item.operation_type)}</td><td>{item.lines.length ? <div className="work-order-material-history-lines">{item.lines.map((line) => <div key={`${item.operation_id}-${line.line_no}`}><span className="mono">{line.material_id}</span><span>{line.quantity} · {line.condition_before}{line.condition_after ? ` → ${line.condition_after}` : ""}</span><small>{line.serials.length ? `SN ${line.serials.length} 条` : "无 SN"} · 账户 <span className="mono">{line.stock_account_id}</span></small>{line.serials.length > 0 && <span className="work-order-material-serial-status">{line.serials.map((serial) => <span key={serial.serial_id} className={serial.sku_verified && serial.qr_verified ? "verified" : "unverified"}>{serial.serial_id} · SKU {serial.sku_verified ? "已核验" : "未核验"} · QR {serial.qr_verified ? "已核验" : "未核验"}</span>)}</span>}</div>)}</div> : "无明细"}</td><td className="mono">{item.posting_transaction_id}</td><td>{item.status === "posted" ? "已过账" : item.status}</td></tr>)}</tbody></table></div></div>;
}
