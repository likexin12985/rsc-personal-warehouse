...
2026-09-06 当前分支 HEAD 可能是文档后继，接管时须重新核验；最新已验收功能代码基线为 `f482b8e`，迁移 head
仍为 `20260905_0062`。`f482b8e` 新增非期初过账命令只读状态查询
`/{task_id}/post-differences-command-status`，使用精确 `X-Request-ID` 重证审计、状态转换、审批封印、
过账完成及库存事实；未新增迁移。该功能的 PostgreSQL 16 release gate 为 run `33982740333`，
