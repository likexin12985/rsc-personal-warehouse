...
2026-09-05 最新已验收代码 `b57f3c3` 已推送，迁移 head 为 `20260905_0062`。
业务提交 `38b35e6` 新增前向迁移 `0062`：日常初盘/复盘在过账、关闭后回查
原计数，使用显式历史冻结证明、完整专用 owner 锁图与递归祖先审计；写侧权限和冻结守卫不变。
本地完整回归3045项通过、1项真库环境跳过（814.07秒）；准确 SHA
PG16 run`33970055640`和客户端run`33970055644`全绿，含清理；PG静态2112、动态复合门禁1项，
Web776、小程序647。
真库专用库位先完成真实零期初6步闭环，再完成三轮日常计数、两级复核、实际+1盘盈、对账和关闭。
此前旧目录caller断言和夹具缺期初资格的失败分别由`1f7322c`、`b57f3c3`修正，未放宽守卫或改成零差异。
范围、准确证据和迁移兼容见[终态历史查询验收](docs/DAILY_COUNT_HISTORY_TERMINAL_ACCEPTANCE.md)。
SN、截止回放、多范围、尾部身份竞争和原 POST 锁序仍需专门真库验收，随后再接两端日常持久恢复；
不代表一期、真实 UAT 或生产放行。
