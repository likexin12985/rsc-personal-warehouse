...
- 正式库存只读接口挂载 `/api/v1/inventory/summary` 与 `/api/v1/inventory/personal/me`，校验
  ...
  这些是当前本地源码的正式契约和最小 ACL 边界，不等于已经完成真实 PostgreSQL 16 并发验收、
  预生产迁移、UAT 或生产授权；未复核/未成立范围的数量继续失败关闭。
...
- `0027` 已在本地源码中以迁移所有者精确图锁收口既有期初启动、实盘、观察处置、复核、复盘和过账
  ...
  这不代表 PostgreSQL 发布验收完成：仍必须在真实 PostgreSQL 16 上执行迁移 SQL、
  `SET ROLE star_oam_api` 全链烟测和双会话语法/并发/死锁/TOCTOU 演练，全部通过前不得放行预生产。
  PC 受控启动也必须等待正式
...
当前唯一 Alembic head 为 `20260904_0054`。`20260903_0052` 的期初盘点请求证据链不支持新旧应用与数据库混合运行；
...
函数 OID、签名、所有者、ACL、六个调用方和触发器绑定保持不变，readiness 精确推进到 `0054`。
`0052`、`0053` 均为不可改写的迁移历史，当前版本不得停留在任一版本。正式升级必须先冻结全部
盘点写入口并排空旧事务，再备份、迁移到 `0054`、一次性替换全部后端、验证 readiness 与隔离 UAT，
最后恢复写入；期初盘点写入事务必须使用 PostgreSQL `READ COMMITTED`，其他隔离级别会被数据库以
`23514` 拒绝。PostgreSQL 的 `0054 -> 0053` 仅允许不存在任何期初任务或保留期初证据的空历史场景；
存在相关事实时必须前向修复或按获批事故流程恢复完整备份，禁止破坏性降级。当前 `0054` 尚未取得
对应准确 Git SHA 的远端 PostgreSQL 16 release-gate 成功证据，不得据此放行。完整顺序与恢复边界见
[0052/0053/0054 期初盘点证据链发布清单](docs/OPENING_STOCKTAKE_0052_RELEASE_RUNBOOK.md)。
