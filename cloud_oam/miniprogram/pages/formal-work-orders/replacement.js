...
module.exports = ({ api, session, scanCode }) => ({
  renderRemovedDraft() {
    ...
    const rows = this._removedDrafts.map(row => {
      ...
      return { id: row.id, basisName: source.material_name, basisSku: source.sku_code,
        ...
        identified: !!candidate, materialName: candidate ? candidate.material_name : '', sku: candidate ? candidate.sku_code : '',
  ...
  editRemoved(event) {
    ...
    if (field !== 'quantity') { row.candidate = null; row.installedSerialId = null }
    this.renderDraft()
  },
  ...
  async inspectRemoved(event) {
    ...
    return this.runReplacementRead(async current => {
      ...
      const candidate = await paired.resolveRemoved({ api, row, expected, current })
      row.candidate = candidate; row.installedSerialId = null
