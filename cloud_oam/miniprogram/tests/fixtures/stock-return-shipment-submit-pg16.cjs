...
async function main() {
  ...
  assert.equal(contract.requestHash(fixture), fixture.pythonRequestHash)
  ...
  global.wx = { getRandomValues: () => crypto.randomBytes(18), request: options => {
    ...
    if (options.method === 'POST' && path.endsWith('/shipments')) {
      ...
      assert.equal(marker.kind, contract.KIND); assert.equal(marker.operation_type, contract.ACTION); assert.equal(marker.operation_id, fixture.operationId)
      assert.equal(marker.trace_request_id, options.header['X-Request-ID']); assert.equal(options.data.request_id, options.header['X-Request-ID'])
      assert.equal(options.data.idempotency_key, options.header['Idempotency-Key']); assert.equal(options.data.expected_plan_hash, marker.plan_hash)
      for (const key of ['reason', 'quantity', 'serial_no', 'qr_code', 'idempotency_key', 'shipped_at', 'token']) assert.ok(![...records.values()].join().includes(key))
  ...
  const args = { api, store, ...fixture, drafts: Object.fromEntries(fixture.lines.map(row => [row.outbound_line_id,
    { quantity: row.quantity, serial_ids: row.serial_ids }])), authorize, confirm: async review => {
  ...
  let result = await submitShipment(args)
  if (fixture.mode !== 'post') {
    ...
    if (fixture.mode === 'lost_before') { assert.equal(result.status, 'pending'); result = await sealPending({ ...args, store: makeStore(), confirm: async () => true }) }
  }
