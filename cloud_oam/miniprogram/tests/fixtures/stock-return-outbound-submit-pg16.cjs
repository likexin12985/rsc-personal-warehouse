...
async function main() {
  ...
  global.wx = { getRandomValues: () => crypto.randomBytes(18), request: options => {
    ...
    if (options.method === 'POST' && path.endsWith('/outbounds')) {
      ...
      for (const key of ['reason', 'quantity', 'serial_no', 'qr_code', 'idempotency_key', 'outbound_at', 'token']) assert.ok(![...records.values()].join().includes(key))
  ...
  const args = { api, store, ...fixture, drafts: Object.fromEntries(fixture.lines.map(row => [row.operation_line_id,
  ...
  let result = await submitDeparture(args)
