...
test('preparation remains read only by default; startup controls require release opening and internal page stays unregistered', () => {
  ...
  assert.match(markup, /启动功能尚未开放，不可启动/)
  ...
  assert.match(markup, /wx:if="\{\{openingStart.enabled && !openingStart.pending\}\}"/)
  assert.equal(require('../utils/opening-start-client').OPENING_START_ENABLED, false)
