...
const { time, canonical } = require('./my-receipt-command')
const PREFIX = 'rsc_oam_my_receipt_v1:'
const FIELDS = ['v', 'kind', 'request_id', 'shipment_id', 'person_id', 'authorization_version', 'expected_request_version', 'received_at', 'trace_request_id', 'request_hash']
const DEFAULT_STATE = { active: new Set(), faults: new Set() }
function fail(message = '验收恢复存储不可用，请保留原记录，暂勿再次提交。') { throw new Error(message) }
function keyOf(value) { return `${uuid(value.request_id)}:${uuid(value.shipment_id)}` }
function validateMarker(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).sort().join('|') !== FIELDS.slice().sort().join('|')
    || value.v !== 1 || value.kind !== 'my_receipt' || !Number.isSafeInteger(value.authorization_version) || value.authorization_version < 1
  ...
  return Object.freeze({ v: 1, kind: 'my_receipt', request_id: uuid(value.request_id), shipment_id: uuid(value.shipment_id),
    person_id: uuid(value.person_id), authorization_version: value.authorization_version,
    expected_request_version: value.expected_request_version, received_at: time(value.received_at),
...
function createStore(options = {}) {
  ...
  return Object.freeze({
    ...
    async withLease(value, work) {
      const id = keyOf(value)
      if (state.active.has(id) || read(value).kind === 'unavailable') fail('该包裹正在处理或恢复记录不可用，请先核验原结果。')
      state.active.add(id)
      let live = true
      const check = marker => {
      ...
      const lease = {
        ...
        persist(marker) {
          ...
          if (read(value).kind !== 'missing') fail('原验收结果未确认，禁止覆盖恢复坐标。')
          try {
          ...
          } catch (_) { state.faults.add(id); fail('验收恢复记录写后核验失败，已停止发送请求。') }
        },
        clearExact(marker) {
