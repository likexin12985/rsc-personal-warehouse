...
function validateItem(value, schemaVersion = '1.0') {
  if (schemaVersion !== '1.0' && schemaVersion !== '2.0') fail('正式物料目录版本不受支持')
  ...
  const updatedAt = schemaVersion === '2.0' && object.source_updated_at === null
    ? null : textValue(object.source_updated_at, 'source_updated_at', 80)
...
function validatePage(value) {
  ...
  if (object.schema_version !== '1.0' && object.schema_version !== '2.0') fail('正式物料目录版本不受支持')
  if (!Array.isArray(object.items)) fail('正式物料目录items无效')
  if (object.items.length > 100) fail('正式物料目录分页超过服务端上限')
  const items = object.items.map((value) => validateItem(value, object.schema_version))
  if (
  ...
  return Object.freeze({ schema_version: object.schema_version, items, next_after_id: nextAfterId })
}
