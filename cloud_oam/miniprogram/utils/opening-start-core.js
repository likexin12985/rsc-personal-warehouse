// Generated by scripts/sync_opening_start_core.mjs; do not edit.
// Source SHA256: 1dfde0ce1ed89c1771b901263f55e51691ef61c86900be7ef0c23852bc72f6fa
...
const START_SEAL_CONFIRMATION = "终结后，该原启动请求将永久禁止执行。如果任务已经创建，只会恢复原任务。确认终结原请求？";
...
const START_STORAGE_PREFIX = "rsc-opening-start-v1:";
const START_LOCK_PREFIX = "rsc-opening-start-region-v1:";
...
class OpeningStartError extends Error {}
class OpeningStartPendingError extends OpeningStartError {
...
function fail(message = "启动数据或恢复记录无法确认，已停止写入")        { throw new OpeningStartError(message); }
...
function startUuid(value         )         { if (typeof value !== "string" || !UUID.test(value)) fail(); return value; }
function integer(value         , min = 0)         { if (typeof value !== "number" || !Number.isSafeInteger(value) || value < min) fail(); return value; }
function label(value         , max        , empty = false)         {
  if (typeof value !== "string" || value.trim() !== value || (!empty && !value) || value.length > max || /[\u0000-\u001f\u007f]/.test(value)) fail();
  return value;
}
function startActor(value         )             {
...
function startActiveIdentity(value         , expected            )       {
  ...
  for (const [key, max] of [['name',160],['employee_no',100],['organization_code',120],['organization_name',240]]         ) label(row[key], max);
...
function validateStartBatchPage(value         , actor            , region        , after                = null) {
...
function validateStartInput(value         )             {
  ...
  const seen = new Set        ();
  const scopes = row.scopes.map((value)             => {
    ...
    if (seen.has(key) || !["hard", "cutoff_replay"].includes(scope.freeze_mode          )) fail();
    seen.add(key);
    const user = label(scope.assignee_user_id, 36);
    if (!REFERENCE.test(user)) fail();
    return Object.freeze({ owner_org_id: owner, location_id: location, assignee_user_id: user, freeze_mode: scope.freeze_mode                              });
  });
...
function validateStartMarker(value         )              {
...
function validateStartRecovery(value         , marker             , actor            )                                 {
...
function createStartStore(storage                     , locks                   )             {
  ...
  return Object.freeze({
    ...
    async withRegionLease(region, work) {
      ...
      return locks.request(START_LOCK_PREFIX + region, { mode: "exclusive", ifAvailable: true }, async (lock) => {
        ...
        const checked = (value             ) => { check(); const marker = validateStartMarker(value); if (marker.region_org_id !== region) fail(); return marker; };
...
function current(canContinue               )       { if (!canContinue()) fail("页面或身份已变化，请重新打开后核验"); }
async function identity(ports            , actor            , live               ) {
  current(live); await ports.identity(actor); current(live);
}
async function recover(lease            , marker             , actor            , ports            , live               ,
  direct                     = null)                        {
  try {
...
async function recoverOpeningStart(options                                                                                                         )                        {
...
async function sealOpeningStart(options
                                                               )                        {
...
function definitiveRejection(value         )          {
...
async function submitOpeningStart(options                                                                                                                              )                        {
...
module.exports = { START_SEAL_CONFIRMATION, START_STORAGE_PREFIX, START_LOCK_PREFIX, OpeningStartError, OpeningStartPendingError, startErrorMessage, startUuid, startActor, startActiveIdentity, validateStartBatchPage, validateStartInput, validateStartMarker, validateStartResult, validateStartRecovery, createStartStore, recoverOpeningStart, sealOpeningStart, submitOpeningStart };
