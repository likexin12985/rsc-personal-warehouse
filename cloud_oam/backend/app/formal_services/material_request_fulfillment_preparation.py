...
0070 has reservation/release facts but no picking consumption facts. Therefore
this reader intentionally refuses requests whose outbound has already started.
A later picking implementation must extend that evidence graph first.
"""
...
def _list(db, *, actor, request_id, line_id):
    ...
    for fact in visible:
        ...
        pool_claim = reserved_totals.get(account.id, ZERO) - release_totals.get(account.id, ZERO)
