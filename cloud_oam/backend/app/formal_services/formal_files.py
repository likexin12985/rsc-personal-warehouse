...
from formal_file_integrity import (
    ...
    _FORBIDDEN_FILENAME_CODEPOINTS,
...
_PERMISSION_BY_PURPOSE: Final[dict[str, tuple[str, str, str]]] = {
    ...
    "opening_count_import": ("stocktake", "count", ""),
...
def _authorize_download(
    ...
    *,
    ...
    if purpose == "source_configuration_evidence":
        # Source reviewers may recover their own evidence through the file API.
        # This does not confer access to the private owner configuration tables.
        if row.uploaded_by != actor.user_id:
            _fail("file_download_forbidden", "forbidden", "当前账号不能下载该来源配置审核证据")
        _require_uploader_continuity(row, row.metadata_jsonb, actor)
        _require_upload_permission(db, actor, purpose)
        return {"binding_type": "source_configuration_evidence", "access": "uploader_current_authority"}
...
def _require_upload_permission(
    ...
    if purpose == "source_configuration_evidence":
        ...
        organization = db.scalar(select(Organization).join(Person, Person.organization_id == Organization.id)
                                 .where(Person.id == actor.person_id))
        allowed = bool(organization and organization.status == "active" and organization.org_type == "headquarters"
                       and any(item.role_code == "admin" and item.scope_type == "national" and item.scope_id == "*"
                               for item in actor.assignments)
                       and any(actor.allows(db, resource, "authorize", target_scope_type="national", target_scope_id="*")
                               for resource in ("inventory_control", "material_source")))
        if not allowed:
            _fail("file_purpose_forbidden", "forbidden", "当前账号不能上传来源配置审核证据")
        return
