...
class DailyMappingDecision(CreatedAtMixin, Base):
    __tablename__ = 'daily_comparison_mapping_decisions'
    __table_args__ = (
        CheckConstraint("action IN ('grant','revoke')", name='ck_daily_mapping_action'),
        CheckConstraint('valid_from>=created_at AND (valid_to IS NULL OR valid_to>valid_from)', name='ck_daily_mapping_validity'),
        CheckConstraint("(action='grant' AND revoked_grant_id IS NULL) OR (action='revoke' AND revoked_grant_id IS NOT NULL AND valid_to IS NULL AND valid_from=created_at)", name='ck_daily_mapping_shape'),
        CheckConstraint('actor_authorization_version>0 AND length(rules_sha256)=64 AND length(payload_sha256)=64 AND length(request_sha256)=64 AND length(review_sha256)=64 AND length(evidence_sha256)=64', name='ck_daily_mapping_proof'),
        UniqueConstraint('actor_user_id','idempotency_key',name='uq_daily_mapping_actor_key'),
        UniqueConstraint('actor_user_id','request_id',name='uq_daily_mapping_actor_request'),
        UniqueConstraint('revoked_grant_id',name='uq_daily_mapping_revocation'),
        Index('uq_daily_mapping_revision','binding_id','catalog_id','rules_revision',unique=True,
              postgresql_where=text("action='grant'"),sqlite_where=text("action='grant'")),
        Index('ix_daily_mapping_subject','binding_id','catalog_id','created_at','id'),
    )
    ...
    revoked_grant_id: Mapped[UUID | None] = mapped_column(UUID_TYPE,ForeignKey('daily_comparison_mapping_decisions.id',ondelete='RESTRICT'),nullable=True)
