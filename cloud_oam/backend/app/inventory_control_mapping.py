"""Current HQ session approval of mapping versions; no master or stock writes."""
...
from .inventory_control_mapping_models import InventoryControlMappingDecision as Decision
...
def _rules(value):
    from .inventory_control_normalization import _rules as normalize_rules
    return normalize_rules(value)[0]
...
def _prove(db,row,seen=None):
    ...
    try:
        ...
        expected=dict(schema_version='rsc.inventory_control_mapping_decision.v1',decision_id=str(row.id),action=row.action,
        ...
        if event.action!='inventory_control.mapping.'+row.action or event.aggregate_type!='inventory_control_mapping_decision' \
            or event.aggregate_id!=str(row.id) or event.actor_user_id!=row.actor_user_id or event.before_jsonb!={} \
            or event.after_jsonb!=_audit_payload(row) or event.request_id!='control-mapping:'+str(row.id) \
...
def _review(db,principal,command):
    ...
    document=dict(schema_version='rsc.inventory_control_mapping_review.v1',actor_user_id=principal.user_id,
...
def execute_inventory_control_mapping(db,*,access_token,expected_authorization_version,command,review_sha256):
    ...
    with db.begin_nested():
        ...
        row.payload_jsonb=dict(schema_version='rsc.inventory_control_mapping_decision.v1',decision_id=str(row.id),action=row.action,
            subject=subject,rules=rules,rules_sha256=row.rules_sha256,revoked_grant_id=str(row.revoked_grant_id) if row.revoked_grant_id else None,
        ...
        audit=append_audit_event(db,stream_key='authorization',actor_user_id=principal.user_id,action='inventory_control.mapping.'+row.action,
            aggregate_type='inventory_control_mapping_decision',aggregate_id=str(row.id),request_id='control-mapping:'+str(row.id),
