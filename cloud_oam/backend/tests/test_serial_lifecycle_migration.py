...
from app.oam_sync_scope_security import OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0092
...
def test_serial_lifecycle_guards_and_update_columns_are_pinned():
    ...
    for coordinate, (_, result, body) in migration["FUNCTIONS"].items():
        if coordinate == ("rsc_check_serial_lifecycle_0092", "uuid"):
            following = runpy.run_path(str(MIGRATION.with_name("20261003_0093_work_order_replacements.py")))
            assert body == following["_sources"]()["public.rsc_check_serial_lifecycle_0092(uuid)"][0]
        else:
    ...
    assert OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0092["rsc_oam_runtime_binding_ready_0044()"][6] == migration["NEW_HASH"]
