"""0095 retains ordinary seal facts and independently guards parent commands."""
import hashlib
...
from app.oam_sync_scope_security import OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0095 as OAM_SYNC_FUNCTION_MANIFEST

MIGRATION=Path(__file__).parents[1]/"alembic/versions/20261005_0095_work_order_replacement_seals.py"
...
def test_parent_seal_sql_parses_and_preserves_the_old_immutable_audit_guards(monkeypatch,operation):
    ...
    assert "work_order_replacements" in statements[1] and "work_order_command_seals" in statements[1]
    ...
    if operation=="upgrade":
        ...
        assert "0095 executed replacement cannot be sealed" in m["CHECK_BODY"]
        assert "0095 sealed replacement cannot execute" in m["CHECK_BODY"]
    else:
        assert any("replacement command seals must be retained" in sql for sql in statements)
        assert any("0095 function source, configuration or ownership drift" in sql for sql in statements)
        ...
        with engine.connect() as db, Operations.context(MigrationContext.configure(db)):
            ...
            db.exec_driver_sql("""INSERT INTO work_order_command_seals
                SELECT '00000000000040008000000000000004',oam_work_order_id,actor_user_id,operator_person_id,
                    authorization_version,'replace',request_id,request_reference,request_hash,sealed_at,created_at
                FROM work_order_command_seals""")
            parent=db.exec_driver_sql("SELECT * FROM work_order_command_seals ORDER BY id").all()
            with pytest.raises(RuntimeError,match="0095 downgrade blocked"):
                m["downgrade"]()
