...
def test_0044_scope_function_manifest_matches_migration_bodies_exactly():
    ...
    ready_body_0060 = ready_body_0059.replace(
        migration_0060["PREVIOUS_SCHEMA_REVISION"],
        migration_0060["revision"],
    )
    ready_body_0061 = ready_body_0060.replace(
        migration_0061["down_revision"],
        migration_0061["revision"],
    )
    ...
    assert hashlib.sha256(ready_body_0059.encode("utf-8")).hexdigest() == (
        scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0059[ready_signature][6]
    )
    assert hashlib.sha256(ready_body_0060.encode("utf-8")).hexdigest() == (
    ...
    assert migration_0060["down_revision"] == migration_0059["revision"]
    assert migration_0060["PREVIOUS_SCHEMA_REVISION"] == migration_0059[
        "revision"
    ]
    assert migration_0061["down_revision"] == migration_0060["revision"]
    for ready_body, expected_revision in (
        ...
        (ready_body_0060, migration_0060["revision"]),
        (ready_body_0061, migration_0061["revision"]),
    ):
        assert (
...
def test_forward_readiness_manifests_match_head_hashes():
    ...
    migration_0063 = runpy.run_path(
        str(migration_root / "20260906_0063_review_command_status.py")
    )
    assert (
    ...
    migration_0069 = runpy.run_path(
        str(migration_root / "20260909_0069_stock_reservations.py")
    )
    assert (
    ...
    assert (
        scope_security.OAM_SYNC_FUNCTION_MANIFEST
        is scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0096
    )
    ...
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0071[ready_signature][6] == migration_0071["RUNTIME_READY_BODY_SHA256_0071"]
    migration_0072 = runpy.run_path(str(migration_root / "20260912_0072_outbound_postings.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0072[ready_signature][6] == migration_0072["RUNTIME_READY_BODY_SHA256_0072"]
    migration_0079 = runpy.run_path(str(migration_root / "20260919_0079_runtime_readiness_head.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0079[ready_signature][6] == migration_0079["RUNTIME_READY_BODY_SHA256_0079"]
    ...
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0084[ready_signature][6] == migration_0084["NEW_HASH"]
    migration_0085 = runpy.run_path(str(migration_root / "20260925_0085_fulfillment_version_commands.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0085[ready_signature][6] == migration_0085["NEW_HASH"]
    migration_0086 = runpy.run_path(str(migration_root / "20260926_0086_receipt_evidence_files.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0086[ready_signature][6] == migration_0086["NEW_HASH"]
    migration_0087 = runpy.run_path(str(migration_root / "20260927_0087_inbound_fulfillment_boundary.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0087[ready_signature][6] == migration_0087["NEW_HASH"]
    migration_0088 = runpy.run_path(str(migration_root / "20260928_0088_receipt_account_admission.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0088[ready_signature][6] == migration_0088["NEW_HASH"]
    migration_0089 = runpy.run_path(str(migration_root / "20260929_0089_personal_inbound_authority.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0089[ready_signature][6] == migration_0089["NEW_HASH"]
    migration_0090 = runpy.run_path(str(migration_root / "20260930_0090_work_order_reservation_causality.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0090[ready_signature][6] == migration_0090["NEW_HASH"]
    migration_0091 = runpy.run_path(str(migration_root / "20261001_0091_work_order_account_admission.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0091[ready_signature][6] == migration_0091["NEW_HASH"]
    migration_0092 = runpy.run_path(str(migration_root / "20261002_0092_serial_consumption_projection.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0092[ready_signature][6] == migration_0092["NEW_HASH"]
    migration_0093 = runpy.run_path(str(migration_root / "20261003_0093_work_order_replacements.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0093[ready_signature][6] == migration_0093["NEW_HASH"]
    migration_0094 = runpy.run_path(str(migration_root / "20261004_0094_work_order_command_seals.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0094[ready_signature][6] == migration_0094["NEW_HASH"]
    migration_0095 = runpy.run_path(str(migration_root / "20261005_0095_work_order_replacement_seals.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0095[ready_signature][6] == migration_0095["NEW_HASH"]
    migration_0096 = runpy.run_path(str(migration_root / "20261006_0096_removed_serial_registrations.py"))
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0096[ready_signature][6] == migration_0096["NEW_HASH"]
    ...
    assert scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0069[ready_signature][6] != (
        scope_security.OAM_SYNC_FUNCTION_MANIFEST_THROUGH_0068[ready_signature][6]
    )
