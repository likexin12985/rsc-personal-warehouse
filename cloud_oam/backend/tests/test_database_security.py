...
def _opening_terminal_0052_function_bodies() -> dict[tuple[str, str], str]:
    ...
    migration_0091 = runpy.run_path(str(Path(__file__).resolve().parents[1] / "alembic/versions/20261001_0091_work_order_account_admission.py"))
    assert hashlib.sha256(hardened_account_body.encode()).hexdigest() == migration_0091["ACCOUNT_OLD_HASH"]
    hardened_account_body = hardened_account_body.replace(migration_0088["ANCHOR"], migration_0091["ACCOUNT_BRANCH"] + migration_0088["ANCHOR"])
...
def test_0046_material_request_guard_catalog_accepts_exact_manifest(
    ...
    assert len(triggers) == 166
    assert len(functions) == 70
    _assert_material_request_approval_guards(
        triggers=triggers,
        functions=functions,
...
def test_0046_material_request_guard_trigger_query_captures_complete_scope(
    ...
    } == {"0029", "0030", "0045", "0046", "0059", "0060", "0069", "0070", "0071", "0072", "0077", "0087", "0090", "0092", "0093", "0094", "0095", "0096"}
...
def test_0045_material_request_approval_function_bodies_match_manifest(
    ...
    assert len(MATERIAL_REQUEST_APPROVAL_FUNCTION_BODY_SHA256) == 70
