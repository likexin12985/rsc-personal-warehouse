"""Exclude sealed parent reversals without weakening ordinary seal proofs."""
import hashlib
...
revision = "20261009_0099"
down_revision = "20261008_0098"
branch_labels = depends_on = None
OLD_HASH = "c3b1e7dbd15f5e98e0e73820fce6d57a14fa7c4be2bcc7029e861f9b825bd61c"
NEW_HASH = "b243cded179bdc3db917929adedc477a32f34eef1d1988319d77529058f0e393"
OLD_CHECK = "operation_type IN ('occupy', 'consume', 'release', 'replace', 'register_removed')"
NEW_CHECK = "operation_type IN ('occupy', 'consume', 'release', 'replace', 'register_removed', 'reverse')"

CHECK_BODY = """
DECLARE
    seal public.work_order_command_seals%ROWTYPE;
    reversal public.work_order_reversals%ROWTYPE;
BEGIN
    PERFORM 1 FROM public.inventory_ledger_heads WHERE stream_key = 'inventory' FOR UPDATE;
    IF NOT FOUND THEN RAISE EXCEPTION '0099 inventory ledger head missing' USING ERRCODE = '23514'; END IF;
    IF TG_TABLE_NAME = 'work_order_command_seals' THEN
        ...
        IF seal.operation_type <> 'reverse' THEN RETURN NULL; END IF;
        ...
        IF EXISTS (SELECT 1 FROM public.work_order_reversals parent_fact
            WHERE parent_fact.actor_user_id = seal.actor_user_id
              AND parent_fact.oam_work_order_id = seal.oam_work_order_id
              AND parent_fact.request_id = seal.request_id) THEN
            RAISE EXCEPTION '0099 executed reversal cannot be sealed' USING ERRCODE = '23514';
        END IF;
    ELSE
        SELECT * INTO STRICT reversal FROM public.work_order_reversals WHERE id = NEW.id;
        IF EXISTS (SELECT 1 FROM public.work_order_command_seals original_seal
            WHERE original_seal.operation_type = 'reverse'
              AND original_seal.actor_user_id = reversal.actor_user_id
              AND original_seal.oam_work_order_id = reversal.oam_work_order_id
              AND original_seal.request_id = reversal.request_id) THEN
            RAISE EXCEPTION '0099 sealed reversal cannot execute' USING ERRCODE = '23514';
        END IF;
    END IF;
    RETURN NULL;
END;
"""
FUNCTIONS = {("rsc_guard_work_order_reversal_seal_0099", ""): ("", "trigger", CHECK_BODY)}
FUNCTION_HASHES = {key: hashlib.sha256(value[2].encode()).hexdigest() for key, value in FUNCTIONS.items()}
TRIGGERS = {
    "trg_work_order_seals_reversal_0099": ("work_order_command_seals", "INSERT", "rsc_guard_work_order_reversal_seal_0099", 5, True),
    "trg_work_order_reversals_seal_0099": ("work_order_reversals", "INSERT", "rsc_guard_work_order_reversal_seal_0099", 5, True),
}
...
def _transition(upgrade):
    ...
    if db.dialect.name not in {"postgresql", "sqlite"}:
        raise RuntimeError("0099 supports PostgreSQL and SQLite only")
    ...
    if db.dialect.name == "postgresql":
        ...
        op.execute("LOCK TABLE public.inventory_ledger_heads, public.inventory_transactions, public.work_order_material_operations, public.work_order_reversals, public.work_order_command_seals, public.audit_events, public.state_transition_events IN SHARE ROW EXCLUSIVE MODE")
    if not upgrade:
        previous["_preflight"]("EXISTS (SELECT 1 FROM work_order_command_seals WHERE operation_type = 'reverse')",
            "0099 downgrade blocked: reversal command seals must be retained")
    if db.dialect.name == "postgresql":
        ...
        if upgrade:
            ...
            for (name, signature), digest in FUNCTION_HASHES.items():
                op.execute(f"""DO $body$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_proc p
                    ...
                    THEN RAISE EXCEPTION '0099 function source, configuration or ownership drift'; END IF; END $body$""")
        ...
        replace(signature="public.rsc_oam_runtime_binding_ready_0044()", expected_hash=OLD_HASH if upgrade else NEW_HASH,
            ...
            replacements=((down_revision, revision),) if upgrade else ((revision, down_revision),), label="work_order_reversal_seal_readiness_0099")
    _constraint(upgrade)
