...
唯一 head 为 `20260905_0059`，前驱为 `20260905_0058`。不得修改任何历史迁移。
...
升级/降级均要求供给事实图完全为空。授权环境中可用以下只读查询预检：
...
任一为真即停止：不得自动删除、补造证据或把原型事实视为正式命令。
需单独审计历史数据并制定前向兼容迁移；本次不提供通用回填。
已产生正式供给事实后禁止降级到 `0058`，只能前向修复或按获批事故流程恢复完整备份。
