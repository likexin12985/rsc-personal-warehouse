...
`addb82350c6597398629168cef4648fb6448d144` 修复所选 manager grant 丢弃完整 principal deny 的
边界。稳定 HTTP 配置原本已由顶层阻断；实际复现的是 service 直接调用，以及请求依赖取得
旧 allow 快照后目录服务加载新 deny 时仍泄露选项。修复前 6 项失败、3 项既有阻断通过；
保留单一精确 manager grant，不合并授权，不扩大数据库权限。

`be3f615` 修复普通人员目录的原始 lookahead 游标泄露：内部每批 100、累计上限 1000，
先筛选足够的授权人员再分页，next 只使用当前页最后返回人员 ID，下一页严格大于该 ID。
超过上限整页 503；候选、发起人及组织/父位置绑定均刷新复验，漂移整页拒绝。
schema/客户端字段不变；升级需从第一页重载，不能复用旧版页面暂存游标。
两项合并后目录/普通盘点写侧定向 116 项通过；根任务扩大复跑共 203 项通过（58.09 秒）。

候选 `75b7e42a988f1178145e91a747b43bba467dac65` 把正式期初详情读取与普通目录单测加入
静态工作流，并增加真实 API 登录 READ ONLY 正向分页、精确 manager 独立允许和跨 grant deny
