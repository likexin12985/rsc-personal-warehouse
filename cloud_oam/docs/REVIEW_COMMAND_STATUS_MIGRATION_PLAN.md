...
状态：0063 前向迁移、应用侧证据链和独立复核命令状态读端已实现；准确提交
`a00f591deb894e748f45806c442ee13ff42432e0` 已通过 Client gate `34021817679`
与 PostgreSQL 16 gate `34021817686`。本文仍不是生产放行证据，也不授权访问 OAM、
RSC、Workflow、飞书或生产数据库。

## 当前实现与门禁证据

- `GET /api/v1/stocktakes/{task_id}/rounds/{round_id}/reviews/{review_stage}/command-status`
  为只读、无缓存、精确请求轨迹接口；请求参数、权限坐标、owner graph、状态迁移、
  审计链和决定摘要任一缺失或篡改均 fail-closed。
- 复核写入、幂等重放、重算来源和 generic 查询均重证 `expected_task_version` /
  `resulting_task_version` 连续性，不猜测旧历史版本。
- PG16 门禁覆盖 0063 复核降级阻断，并分别验证正常 Alembic 顺序中的 0055/0063
  优先级和 0047/0061 迁移函数直接调用；历史迁移文件未被改写。

剩余范围：一期需求供给纵向链路尚未开始；审批、分配、占用、出库、发货、物流签收、
OAM 收货、RSC/个人仓入库、通知送达和对账同步继续保持独立未完成状态轴。
