...
当前新增批次为 0072 实物出库：原拣货与出库单快照保持不可变，独立出库事实证明
数量/SN 从拣货中转入在途；Web 页面和断线恢复已接入。
当前实现与准确门禁证据见[实物出库验收](OUTBOUND_POSTING_ACCEPTANCE.md)。
代码 `4a6ec83` 的 Client `34215778415` 与完整 PG16 `34215778501` 均为 success。
真库迁移与并发 `1 passed`，后端/边缘门禁回归 `2416 passed, 2 skipped`，
Web `1009`、小程序及门禁契约 `724` 项通过；文档后继提交不构成新的后端迁移证据。
后续顺序为独立发运/分包、物流事件、分批验收与个人仓入账；发运登记不得重复扣库存。
第 8 节已更新为 0072 后的开发顺序。以下内容保留各批历史状态，接续时以本段和当前 HEAD 为准。

### 2026-09-08 历史批次记录

0071 批次为正式拣货确认：已增加独立出库单及原占用拣货事实、Web 拣货页面和原请求
恢复。准确实现与测试证据见[拣货确认验收记录](PICKING_CONFIRMATION_ACCEPTANCE.md)。
代码 `24a65cc` 的 Client `34207781014` 与完整 PG16 `34207780991` 均已通过。
该批次当时的下一阶段是实物出库过账及后续发运/收货链；以下段落是更早批次的历史快照。

最新接续增加只读履约准备接口与需求详情面板，核对原占用、释放、剩余数量、SN 和异常原因。
新提交范围与门禁证据见[履约准备验收记录](FULFILLMENT_PREPARATION_ACCEPTANCE.md)。
实际拣货、出库及过账仍须继续实现；当前 head 仍为 0070，不要将准备清单当作履约已经完成。
代码 `07719af3f6fa97b5b6fa6685167c65c69f665e88` 的 Client `34197690785` 与完整
PG16 `34197690731` 均为 success，测试统计和后续过账约束已记录在上述验收文件。

最新接续已实现库存占用释放的 0070 迁移、补偿过账、数量/SN 页面及原请求只读恢复。
当前范围、准确验证结果和后续拣货/出库任务见[释放验收记录](RESERVATION_RELEASE_ACCEPTANCE.md)。
准确代码 `30b67c21074f46f9547360d9bf8aa1574665de09` 的 Client `34192366854` 与完整
PG16 `34192366880` 均为 success，后续文档提交不作为新的后端迁移证据。
以下占用阶段记录保留作历史证据，不能代替新提交的 PG16 结果。

继续使用 `codex/production-readiness-gates`。分配页面已接入，本轮新增只读占用候选、
数量/SN 占用页面和严格的断线恢复证据，同时修复 0069 迁移源码替换冲突及其运行时哈希。
详见[占用页面验收记录](RESERVATION_PAGE_ACCEPTANCE.md)。备案办理与开发并行推进，
占用专用真库验收已完成；后续释放及履约链、生产验收仍须继续。

最新接续已补上分配受限角色行锁、需求范围授权、成立货源候选和占用启动目录检查。
代码 `816a939` 的数量/SN 真库业务验收及完整 Client/PG16 门禁已通过，
准确证据与下一步释放范围统一维护于上方验收记录。
第 8 节接管提示词已更新，避免继续使用旧 0062 阶段的开发顺序。

以下为对应提交的历史记录，不能把旧 SHA 的绿灯、旧的“当前分支”或历史功能缺口
当作当前状态。接管时先核验实际分支、HEAD、工作树、远端及准确提交的 Client/PG16 结果。

### 2026-09-06 最新客户端提交：`83f2223`

本地与远端分支当前均为 `83f2223d8fa5031718cb5b8250c3b2796d4667fd`。
该提交将 Web 日常非期初范围计数（初盘/复盘）和两阶段复核接入持久恢复与只读
command-status 核验；Client gate
[34025174245](https://github.com/likexin12985/rsc-personal-warehouse/actions/runs/34025174245)
全绿（Web `795`、小程序 `670`、类型、构建和安全检查）。代码只改客户端，未访问或写入
OAM、RSC、Workflow、飞书、生产数据库或部署系统；保护目录仍未跟踪。

后端/迁移未变更，PG16 真库证据继续引用后端准确 SHA 的
[34021817686](https://github.com/likexin12985/rsc-personal-warehouse/actions/runs/34021817686)，
不能把客户端门禁当成新的 PG16 结果。小程序非期初过账/复核恢复、服务端未执行封存、正式
控制投影、启动前恢复锚点和供给任务纵向链路仍未完成；十个履约/通知/对账状态轴继续分开。
...
> 请在当前仓库继续 RSC 个人仓开发。先完整阅读根目录 AGENTS.md、V1.0 正式需求与架构
> 文档以及 cloud_oam/README.md。使用 codex/production-readiness-gates，以实际 HEAD、
> 工作树、远端和准确 SHA 的 Client/PG16 门禁重新确认状态，不回退到历史聊天中的旧提交。
> 当前迁移 head 为 20260912_0072；分配、占用、释放、拣货及实物出库服务、Web 页面和断线恢复已经接入。
> 实物出库从拣货中转入明确绑定的在途账户；最新范围和真库证据以 cloud_oam/docs/OUTBOUND_POSTING_ACCEPTANCE.md 为准。
> 先核验准确提交的真实 PG16 出库结果，再继续独立发运/分包、物流事件、工程师分批验收和个人仓入账。
> 发运必须绑定原出库事实并限制累计数量/SN，登记运单不得再次扣减库存。
> 原占用及拣货单快照不可改写；释放、拣货与出库分别使用独立事实和库存流水。审批、分配、占用、拣货、出库、发运、
> 签收、OAM 收货、个人仓入库、通知和对账继续分别保存和验收。
> 开发与备案进度并行推进，备案通过不能代替真实身份/附件 UAT、预生产迁移恢复和发布验收。
> 运行仓库安全检查后再提交和推送，不纳入受保护的部署目录，不访问或写入外部生产系统。
> 下方和前文的旧日期记录只用于追溯，不作为重新开发已完成功能的指令。
