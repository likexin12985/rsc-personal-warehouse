...
- Alembic 当前唯一 head 为 `20260901_0040`。
...
- 仓库安全检查通过：允许进入私有仓库的候选文件为 483 个，合计 14,273,304 字节（约 13.6 MiB）；
  未发现越界路径、
  Git 可见符号链接或子模块、高置信凭据、真实用户绝对路径、已忽略但仍被跟踪的文件。
- Alembic 仅有一个 head：`20260901_0040`。
- FastAPI 后端全量 `1659 passed`；边缘同步单独 `21 passed`。
...
上述计数包含 `0040`、KMS/pin/readiness 门禁、SDK 日志封闭、跨用途 Key 隔离和微信 provider
...
恢复仍是发布门禁。短信挑战的 `sending/accepted/uncertain` dispatch ownership 尚未实现，且
现有 PNVS 接口与用户购买的 1000 条标准短信套餐是否兼容、同 `out_id` 是否严格去重均未确认，
生产短信继续关闭。
客户端发布顺序必须是：先完成全部后端实例的 `0040`（包含 `0039`）迁移、pin gate 与新版切换，
再发布启用恢复哨兵的 PC/小程序。
