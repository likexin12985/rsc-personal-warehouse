...
> 当前迁移 head 为 20260910_0070；分配、占用及释放服务、Web 页面、候选与断线恢复已经接入。
> 履约准备只读清单已接入；最新实施范围和真库证据以 cloud_oam/docs/FULFILLMENT_PREPARATION_ACCEPTANCE.md 为准。
> 先核验准确提交的真实 PG16 分配/占用/释放结果，再继续拣货、出库、发运、签收、
> OAM 收货和个人仓入库，并补齐供给服务与已占用/释放状态组合的兼容验收。
> 原占用记录不可改写；释放使用独立事实及反向库存流水。审批、分配、占用、出库、发运、
