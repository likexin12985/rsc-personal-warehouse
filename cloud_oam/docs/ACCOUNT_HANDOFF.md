...
下文和旧续开发提示中的 0071 是上一批已验收快照，接续时以本段和当前 HEAD 为准。

最新批次为正式拣货确认：已增加 0071、独立出库单及原占用拣货事实、Web 拣货页面和原请求
恢复。准确实现与测试证据见[拣货确认验收记录](PICKING_CONFIRMATION_ACCEPTANCE.md)。
代码 `24a65cc` 的 Client `34207781014` 与完整 PG16 `34207780991` 均已通过。
下一阶段是实物出库过账及后续发运/收货链；以下准备清单段落是上一批的历史快照。
...
> 当前迁移 head 为 20260911_0071；分配、占用、释放及拣货服务、Web 页面、候选与断线恢复已经接入。
> 拣货已生成独立出库单，但尚未实物出库；最新范围和真库证据以 cloud_oam/docs/PICKING_CONFIRMATION_ACCEPTANCE.md 为准。
> 先核验准确提交的真实 PG16 分配/占用/释放/拣货结果，再继续实物出库过账、发运、签收、
> OAM 收货和个人仓入库，并补齐供给服务与已占用/释放状态组合的兼容验收。
> 原占用记录不可改写；释放与拣货分别使用独立事实和库存流水。审批、分配、占用、拣货、出库、发运、
