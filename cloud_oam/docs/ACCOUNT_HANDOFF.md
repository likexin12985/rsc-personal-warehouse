...
当前最新完整 PostgreSQL 16 验收代码为 `e54765a10b4d465c6073df69e3fa97296ef90f93`，
run `33931135022` 全绿。后续依赖声明收口 `2e18820` 及目录权限修复 `addb823` 属独立提交，
前者已离线重装验证；后者及安全分页 `be3f615` 已纳入新验收候选
`75b7e42a988f1178145e91a747b43bba467dac65`，对应 run `33932385447` 正在运行。
新候选相关本地回归 203 项通过（58.09 秒），但尚不能把下面历史绿灯算给当前 HEAD。
...
回滚并核对原目录。当前仅静态/单元通过，这些新增真库断言需等待新 run。
