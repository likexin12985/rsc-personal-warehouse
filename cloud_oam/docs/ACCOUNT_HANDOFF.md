...
- Alembic 当前唯一 head 为 `20260901_0039`。
...
- 当前登录验证码代码接入的是阿里云号码认证服务 `dypnsapi` 的
  ...
  提供套餐所属产品名称及适用接口的非敏感截图或文字信息后再决定是否兼容。不要提供账号密码。
...
- 仓库安全检查通过：允许进入私有仓库的候选文件为 473 个，合计 14,077,004 字节（约 13.4 MiB）；
  未发现越界路径、
  Git 可见符号链接或子模块、高置信凭据、真实用户绝对路径、已忽略但仍被跟踪的文件。
- Alembic 仅有一个 head：`20260901_0039`。
- FastAPI 后端全量 `1577 passed`；边缘同步单独 `21 passed`。
...
发布顺序必须是：先完成全部后端实例的 `0039` 迁移与新版切换，再发布启用恢复哨兵的 PC/小程序。
