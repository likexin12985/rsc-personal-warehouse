...
- Alembic 当前唯一 head 为 `20260903_0052`。
- `0052` 已为期初盘点范围完成事实增加不可变 `request_jsonb` 原始请求和
  ...
  数据库混合运行，正式顺序见 `docs/OPENING_STOCKTAKE_0052_RELEASE_RUNBOOK.md`。
...
2026-09-04 当前 `0052` 工作树在不访问任何外部生产系统的前提下完成：

- 期初盘点、入账和审计链回归 `254 passed`。
- Alembic 迁移、数据库权限及部署安全回归 `430 passed`。
- 与 PostgreSQL 16 GitHub 工作流一致的静态发布测试清单 `965 passed`。
- FastAPI 后端全量 `2058 passed, 1 skipped`；边缘同步全量 `36 passed`。
- Python 编译、`git diff --check`、单一 Alembic head 和仓库安全检查通过。
- 本机没有受控 PostgreSQL 16。`0051 -> 0052` 历史回填、双会话并发、直接 SQL 篡改、数值溢出、
  角色/ACL 和函数体哈希仍必须以本次准确 Git SHA 触发的新 GitHub release-gate 成功为最终证据。
