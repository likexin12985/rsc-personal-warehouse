...
> 当前迁移 head 为 20260909_0069；分配及库存占用服务、Web 页面、候选与断线恢复已经接入。
> 最新实施范围和真库证据以 cloud_oam/docs/RESERVATION_PAGE_ACCEPTANCE.md 为准。
> 先核验准确提交的真实 PG16 分配/占用结果，然后通过新前向迁移实现部分/全部释放、
> SN 释放及恢复，再继续拣货、出库、发运、签收、OAM 收货和个人仓入库。
