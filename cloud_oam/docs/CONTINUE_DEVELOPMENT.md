...
最新实际 HEAD：`b8147d0`（`fix: refresh inbound status after posting`）。
...
本批本地验证：正式 Web 测试 53 个文件/1017 个用例通过，TypeScript 检查和 Vite production build 通过，后端完整测试 3353 passed、2 skipped，仓库安全检查通过；
