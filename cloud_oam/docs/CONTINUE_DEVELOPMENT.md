...
正式操作命令现支持可选的新旧 SN 配对；配对要求新旧不同、安装侧和拆下侧各自不重复，且配对
内容纳入请求指纹。通过后与操作明细在同一事务追加到 `work_order_replacement_pairs`。
...
下一步仍须：实现原子占用/消耗/释放/回收命令、对应权限迁移、业务审计及 Outbox、
以换代修双向流水配对、完整回滚/并发测试和准确 SHA 的 PG16 门禁。保留上述目标，
