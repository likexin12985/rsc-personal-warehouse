...
## 2026-09-12 收货投影生产权限门禁

新增前向迁移 `20260922_0082_oam_receipt_projector_boundary`，为收货投影器建立独立的
`oam_receipt_sync_scope_bindings` 范围表、收货专用 `SECURITY DEFINER` RLS 检查函数、
FORCE RLS 策略和最小 ACL。投影器只能读取收货镜像、只读来源、正式收货对象/发运绑定和
目标发运单，并只能按明确列白名单插入不可变 `oam_receipt_evidence`；不会写本地收货、个人仓
入账、库存、通知或对账事实。生产入口现在调用专用运行时证明，缺少迁移、绑定或精确策略时
继续 fail-closed。迁移降级在存在收货范围绑定时阻断；SQLite 仅保持结构回归兼容。

本批需完成本地后端迁移/安全回归、仓库安全检查和准确 SHA 的 Client/PG16 门禁；PG16 动态
门禁通过后，才可把收货投影器视为具备生产数据库权限基础。真实 OAM 仍只读，范围绑定和
备案推进仍需部署环境按同一坐标配置。
