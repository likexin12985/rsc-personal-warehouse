...
这仍是云端已完成镜像到不可变 `oam_receipt_evidence` 的只读边界，不调用 OAM，不推进本地收货、个人仓入账、通知或对账；独立收货投影 worker 及其 PostgreSQL 16/RLS 运行隔离仍需下一批实现和门禁验证。
...
生产环境当前对该入口保持 fail-closed：既有工单投影 ACL 证明不能推导收货投影权限，必须先完成收货专用
PostgreSQL RLS/ACL 图和准确 SHA 的 PG16 验证。SQLite/开发环境只用于契约回归，不代表生产 worker 已可运行。
