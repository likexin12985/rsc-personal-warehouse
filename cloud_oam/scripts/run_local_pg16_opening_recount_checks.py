#!/usr/bin/env python3
"""Build-independent local actual recount checks in a new owned native PG16 cluster.
...
def main(argv=None):
    ...
    report=None
    try:
        with native_cluster(postgres_bin=args.postgres_bin,artifact_root=CLOUD/'artifacts/opening-recount-20260920/checks') as (directory,engines):
            report=directory
            ...
            for label, command in (('migration', ['upgrade','head']),
                ('empty-down-0127',['downgrade','20261105_0126']),('empty-up-0127',['upgrade','head'])):
            ...
            from pg16_opening_recount_gate import run
            result=run(engines)
            from pg16_opening_recount_gate import snapshot as all_reconciliation_facts
            from sqlalchemy import text
            before=all_reconciliation_facts(engines['star_oam_migrator'])
            for label,command in (('populated-down-0127',['downgrade','20261105_0126']),('populated-up-0127',['upgrade','head'])):
                with (directory/(label+'.log')).open('wb') as output:
                    subprocess.run([sys.executable,'-m','alembic','-c','alembic.ini',*command],cwd=CLOUD,
                        env=environment,stdout=output,stderr=subprocess.STDOUT,timeout=180,check=True)
                if all_reconciliation_facts(engines['star_oam_migrator'])!=before:
                    raise RuntimeError('0127 roundtrip changed reconciliation or stock facts')
            ...
            result['populated0127RoundtripPreservesFacts']=True
